...
- LSP servers are managed via `neovim/nvim-lspconfig`'s new `vim.lsp.config`/`vim.lsp.enable` API (see `lua/plugins/lsp.lua`), not mason.nvim — this config has no mason.nvim plugin. Servers must already be resolvable on `$PATH` (this repo's `init.lua` appends `~/.local/bin`); most are installed via the host's nix profile, except Python's, which is installed with `uv tool install basedpyright` (lands in `~/.local/bin`).
- Python uses `basedpyright`, not plain `pyright`: plain pyright's open-source language server does not implement `textDocument/inlayHint` at all (`server_capabilities.inlayHintProvider` is nil), so it can never show inlay type hints. `ruff` (`ruff server`) runs alongside it for linting.
